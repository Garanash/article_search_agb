...
async def search_email_perplexity(company_name: str, website: str, region: str) -> str:
    prompt = (
        f"Найди максимально релевантный рабочий email для отдела продаж, закупок или оптовых заказов компании {company_name} (сайт: {website}) в регионе {region}. "
        f"Если есть email для sales, order, wholesale, закупок, оптовых или коммерческих запросов — верни его. Если нет — верни любой рабочий email компании. Верни только email (строкой) или пустую строку, если email не найден."
    )
    messages = [
        {
            "role": "system",
            "content": (
                "Ты искусственный интеллект, который помогает находить рабочие email компаний для покупки запчастей. "
                "Твоя задача — найти email для отдела продаж, закупок, оптовых или коммерческих запросов. Если есть — верни его, если нет — верни любой рабочий email. Верни только email (строкой) или пустую строку."
            ),
        },
        {
            "role": "user",
