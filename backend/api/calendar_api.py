...
def get_events(db: Session = Depends(get_db)):
    events = db.execute(text("SELECT * FROM calendar_events ORDER BY start_date DESC")).fetchall()
...
def create_event(event: EventCreate, current_user: User = Depends(auth.get_current_user), db: Session = Depends(get_db)):
    now = datetime.utcnow()
    result = db.execute(
        text("INSERT INTO calendar_events (title, description, start_date, end_date, created_at, updated_at, user_id) VALUES (:title, :description, :start_date, :end_date, :created_at, :updated_at, :user_id) RETURNING *"),
...
def get_one_event(event_id: int, db: Session = Depends(get_db)):
    row = db.execute(text("SELECT * FROM calendar_events WHERE id=:id"), {"id": event_id}).fetchone()
    if not row:
...
def update_event(event_id: int, event: EventUpdate, current_user: User = Depends(auth.get_current_user), db: Session = Depends(get_db)):
    ...
    if not row:
        raise HTTPException(status_code=404, detail="Событие не найдено")
    db.execute(
        text("UPDATE calendar_events SET title=:title, description=:description, start_date=:start_date, end_date=:end_date, updated_at=:updated_at WHERE id=:id"),
