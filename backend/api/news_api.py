...
def get_news(db: Session = Depends(get_db)):
    news = db.execute(text("SELECT * FROM news ORDER BY created_at DESC")).fetchall()
...
def create_news(news: NewsCreate, current_user: User = Depends(auth.get_current_user), db: Session = Depends(get_db)):
    now = datetime.utcnow()
    result = db.execute(
        text("INSERT INTO news (title, text, image_url, created_at, updated_at, author_id) VALUES (:title, :text, :image_url, :created_at, :updated_at, :author_id) RETURNING *"),
...
def update_news(news_id: int, news: NewsUpdate, current_user: User = Depends(auth.get_current_user), db: Session = Depends(get_db)):
    ...
    if not row:
        raise HTTPException(status_code=404, detail="Новость не найдена")
    db.execute(
        text("UPDATE news SET title=:title, text=:text, image_url=:image_url, updated_at=:updated_at WHERE id=:id"),
